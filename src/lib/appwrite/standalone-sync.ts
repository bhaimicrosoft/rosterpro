...
// Load environment variab//li-like functions for easy execution
export const userSync = {
  // Show statistics
  stats: getSyncStats,
  
  // Dry run sync (shows what would happen)
  dryRun: () => syncUsersFromAuthStandalone(true),
  
  // Live sync with default settings
  sync: () => syncUsersFromAuthStandalone(false),
  
  // Sync with custom role and manager
  syncWithRole: (role: 'EMPLOYEE' | 'MANAGER' | 'ADMIN', managerId?: string) => 
    syncUsersFromAuthStandalone(false, role, managerId),
};

// Export for CLI usage
export { syncUsersFromAuthStandalone as syncUsersFromAuth };({ path: path.join(process.cwd(), '.env.local') });
 ...
 */
async function syncUsersFromAuthStandalone(
